...
def main():
    """颜色分类
    """
    Mask = sio.loadmat('Bayesian_decision/tmp/Mask.mat')['Mask']
    training_data = sio.loadmat('Bayesian_decision/tmp/array_sample.mat')['array_sample']
    lable = training_data[:,[4]].flatten() # 标签

    ### 1. 测试灰度图
    img_gray = cv2.imread("Bayesian_decision/tmp/309.bmp", 0)  # 得到灰度图
    ...
    ### 2. 测试rgb图
    img_rgb = cv2.imread("Bayesian_decision/tmp/309.bmp", 1)   # 得到BGR图
